...
export function EditVanModal({
  ...
  // Sync form data when modal opens or van changes
  useEffect(() => {
    if (isOpen && van) {
      setFormData({
        name: van.name || "",
      ...
      setImagePreview(van.image || null);
      setError("");
    }
  }, [isOpen, van]);
